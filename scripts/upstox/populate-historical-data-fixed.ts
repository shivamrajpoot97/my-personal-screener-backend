...
class HistoricalDataPopulator {
  ...
  public async populateHistoricalData(): Promise<void> {
    try {
      ...
      // Get all equity stocks with proper typing
      const equityStocks = await Stock.find({
        instrumentType: 'EQ',
        isActive: true,
        instrumentKey: { $exists: true, $ne: null }
      } as any).select('symbol instrumentKey name').exec();
  ...
  private async executePhase(phaseKey: keyof typeof POPULATION_PHASES, stocks: any[]): Promise<void> {
    ...
    if (!phase || !phase.timeframe) {
      logger.error(`Invalid phase: ${phaseKey}`);
      return;
    }
...
async function main() {
  try {
    ...
    if (!accessToken) {
      console.error('Usage: ts-node populate-historical-data-fixed.ts --token <ACCESS_TOKEN> [options]');
